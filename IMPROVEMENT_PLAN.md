...
* **Action:** In `subtitle_ai/gpu.py`, implement a pre-allocation check using `torch.cuda.mem_get_info()`. If available VRAM is below a safe margin (e.g., < 3.2 GB), sleep and retry for a bounded window (15–30s) before raising an exception.
